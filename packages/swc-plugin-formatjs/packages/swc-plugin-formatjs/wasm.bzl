"""
Macro to cross compile Rust library to WASM.
...
rust_wasm_transition = transition(
...
rust_wasi_transition = transition(
...
rust_wasm_library_rule = rule(
    implementation = _wasm_library_impl,
    attrs = _wasm_attrs(rust_wasm_transition),
)

rust_wasi_library_rule = rule(
    implementation = _wasm_library_impl,
    attrs = _wasm_attrs(rust_wasi_transition),
)
...
def rust_wasm_library(name, tags = [], wasi = False, **kwargs):
    ...
    bin_rule = rust_wasm_library_rule
    if wasi:
        bin_rule = rust_wasi_library_rule
